...
class MahasiswaService
{
    ...
    /**
     * Mengurutkan data mahasiswa berdasarkan field dan algoritma yang dipilih.
     */
    public function sortData(array $mahasiswas, string $field, string $algorithm): array
    {
        $n = count($mahasiswas);
        if ($n <= 1) {
            return $mahasiswas;
        ...
        $compare = function (Mahasiswa $a, Mahasiswa $b) use ($field) {
            $valA = match($field) {
                'nim' => $a->getNim(),
                'nama' => $a->getNama(),
        ...
        if ($algorithm === 'bubble') {
            for ($i = 0; $i < $n; $i++) {
                for ($j = 0; $j < $n - $i - 1; $j++) {
                    if ($compare($mahasiswas[$j], $mahasiswas[$j + 1]) > 0) {
        ...
        if ($algorithm === 'selection') {
            for ($i = 0; $i < $n - 1; $i++) {
                $minIndex = $i;
                for ($j = $i + 1; $j < $n; $j++) {
                    if ($compare($mahasiswas[$j], $mahasiswas[$minIndex]) < 0) {
                        $minIndex = $j;
                    }
                }
                if ($minIndex !== $i) {
    ...
    {
        $sampleData = [
            ['2401011450590', 'Ayu Lestari', 'Teknik Informatika', 3.75, 'ayu@example.com', '081234567801'],
            ['2401011450591', 'Budi Santoso', 'Sistem Informasi', 3.60, 'budi@example.com', '081234567802'],
            ['2401011450592', 'Citra Dewi', 'Teknik Informatika', 3.85, 'citra@example.com', '081234567803'],
            ['2401011450593', 'Doni Pratama', 'Manajemen Informatika', 3.50, 'doni@example.com', '081234567804'],
            ['2401011450594', 'Eka Putri', 'Teknik Komputer', 3.70, 'eka@example.com', '081234567805'],
            ['2401011450595', 'Fajar Nugroho', 'Sistem Informasi', 3.65, 'fajar@example.com', '081234567806'],
            ['2401011450596', 'Gita Ramadhani', 'Teknik Informatika', 3.90, 'gita@example.com', '081234567807'],
            ['2401011450597', 'Hendra Wijaya', 'Teknik Elektro', 3.55, 'hendra@example.com', '081234567808'],
            ['2401011450598', 'Indah Permata', 'Manajemen Informatika', 3.80, 'indah@example.com', '081234567809'],
            ['2401011450599', 'Joko Widodo', 'Sistem Informasi', 3.45, 'joko@example.com', '081234567810'],
        ];
