...
<head>
    ...
    <style>
        body { font-family: Arial, sans-serif; margin: 0; padding: 1rem; min-height: 100vh; background: url('{{ asset('background.jpg') }}') center/cover no-repeat; background-color: #eef2ff; color: #1f2937; }
...
<body>
    ...
    <div class="card">
        ...
        <div class="header-actions">
            <a href="{{ route('mahasiswa.create') }}" class="btn">Tambah Data</a>
            <div class="sort-actions" style="display:flex; flex-wrap:wrap; gap:0.75rem; align-items:center;">
                ...
                <a href="{{ route('mahasiswa.index', array_merge(request()->query(), ['sort' => 'nim'])) }}" class="btn {{ $sort === 'nim' ? 'active' : '' }}">NIM</a>
                <a href="{{ route('mahasiswa.index', array_merge(request()->query(), ['sort' => 'nama'])) }}" class="btn {{ $sort === 'nama' ? 'active' : '' }}">Nama</a>
                <a href="{{ route('mahasiswa.index', array_merge(request()->query(), ['sort' => 'ipk'])) }}" class="btn {{ $sort === 'ipk' ? 'active' : '' }}">IPK</a>
                <a href="{{ route('mahasiswa.index', array_merge(request()->query(), ['sort' => 'jurusan'])) }}" class="btn {{ $sort === 'jurusan' ? 'active' : '' }}">Jurusan</a>
                <a href="{{ route('mahasiswa.index', array_merge(request()->query(), ['sort' => 'email'])) }}" class="btn {{ $sort === 'email' ? 'active' : '' }}">Email</a>
            </div>
            <form action="{{ route('mahasiswa.export') }}" method="GET" style="display:inline;">
    ...
    @if(session('success'))
        <div class="card notice" style="background: #ecfdf3; color: #047857;">{{ session('success') }}</div>
    @endif
    ...
    <div class="card">
        ...
        <form method="GET" action="{{ route('mahasiswa.index') }}" style="margin-bottom: 1rem;">
            <div class="search-row">
                <input name="keyword" placeholder="Cari berdasarkan NIM, nama, jurusan, IPK, atau email" value="{{ $keyword }}">
        ...
        @if($sort)
            <div class="notice" style="background: #eff6ff; color: #1d4ed8; margin-bottom: 1rem;">
                <strong>Urutan aktif:</strong> {{ strtoupper($sort) }}
            </div>
        @endif
        ...
        @if($keyword !== '')
            @if(count($mahasiswas) > 0)
                <div class="notice" style="background: #ecfdf3; padding: 0.9rem; border-radius: 12px; margin-bottom: 1rem;">
                    <strong>Hasil pencarian:</strong> ditemukan {{ count($mahasiswas) }} data untuk '{{ $keyword }}'.
                </div>
            @else
                <div class="notice" style="background: #fef2f2; padding: 0.9rem; border-radius: 12px; margin-bottom: 1rem;">
                    Tidak ada data mahasiswa yang cocok dengan '{{ $keyword }}'.
                </div>
            @endif
        @endif

        <h2>Import Data Mahasiswa</h2>
        <form action="{{ route('mahasiswa.import') }}" method="POST" enctype="multipart/form-data" style="margin-bottom: 1rem;">
            @csrf
            <div class="search-row">
        ...
        <table>
            ...
            @forelse($mahasiswas as $m)
                <tr>
                    ...
                    <td>{{ $m->getEmail() }}</td>
                    <td>
                        <a href="{{ route('mahasiswa.edit', $m->getNim()) }}" class="btn">Edit</a>
                        <form action="{{ route('mahasiswa.destroy', $m->getNim()) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
